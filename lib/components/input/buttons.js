...
export default class Buttons {
        ...
        this.#instances = Array.from({ length: MAX_COLORS }, (_, color) =>
            Array.from({ length: TYPES }, (_, type) => {
                ...
                for (let index = 0; index < MAX_INSTANCES; index++) {
                    arr.push(createInstance({ color, type, index }));
                }
                return arr;
            })
        );
    ...
    static deposit({ position, rotation, color = 0, type = 0, onPress } = {}) {
        ...
        this.#scene.addObject ? this.#scene.addObject(bulbLight) : this.#scene.add?.(bulbLight);
    ...
    // ALIAS : Permet d'éviter l'erreur rI.addButton
    ...
    static update() {
        for (let c = 0; c < MAX_COLORS; c++) {
            for (let t = 0; t < TYPES; t++) {
                for (const instance of this.#instances[c][t]) {
                    if (instance.used) {
