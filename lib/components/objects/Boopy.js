...
export default class {
    ...
    static save() {
        return this.#instances.map(instance => {
            return {
                ...
                used: instance.used,
                bodyHandle: instance.body.handle
            };
        });
    }
    ...
    static load(boopies) {
        if (!boopies) return;
        boopies.forEach((data, instanceIndex) => {
            const boopy = this.#instances[instanceIndex];
            if (!boopy) return;

            const body = boopy.body;

            boopy.used = data.used;
            boopy.position.fromArray(data.position);
            boopy.rotation.fromArray(data.rotation);

            body.setTranslation(boopy.position, true);
            body.setRotation(boopy.rotation, true);
            body.setEnabled(data.used);
...
async function initializeModel({ scene }) {
    ...
    model.scene.traverse((child) => {
        if (child.isMesh) {
            ...
            // Aligne le centre de la géométrie au centre de la bounding box
...
function createInstance({ scene, instances, groups }) {
    ...
    // Collider Cuboid stable et performant
