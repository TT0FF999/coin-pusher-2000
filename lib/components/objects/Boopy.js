...
const INITIAL_POSITION = [0, 0.6, 0.5];
...
export default class {
    ...
    static async initialize({ scene, groups }) {
        ...
        // Mise à jour des dimensions réelles basées sur le modèle boopy.glb
        WIDTH = width;
        HEIGHT = height;
        DEPTH = depth;
        ...
        const mergedGeom = scene.mergeGeometries(geometries);
        createInstances({
            scene,
            instances: this.#instances,
            colliderData: mergedGeom,
            groups
        });
    }
...
// Extraction dynamique et normalisation des maillages du modèle GLB
async function initializeModel({ scene }) {
    const model = await scene.loadModel(MODEL_PATH);
    const materials = [];
    const geometries = [];

    // Mettre à jour les matrices du modèle 3D
    model.scene.updateMatrixWorld(true);

    // Calcul de la BoundingBox globale pour centrer l'objet et connaître ses vraies dimensions
    const globalBox = new Box3().setFromObject(model.scene);
    ...
    // Parcourt tous les éléments 3D et applique leurs transformations relatives
    model.scene.traverse((child) => {
        if (child.isMesh) {
            materials.push(child.material);
            const geom = child.geometry.clone();
            
            // Applique les transformations de Blender/GLTF (position, rotation, échelle)
            geom.applyMatrix4(child.matrixWorld);
            
            // Recentre la géométrie sur (0,0,0) pour que le centre physique corresponde au centre visuel
