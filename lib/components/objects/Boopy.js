...
const ADDITIONAL_SOLVER_ITERATIONS = 4;
...
export default class {
    ...
    static async initialize({ scene, groups }) {
        this.#scene = scene;
        const { materials, geometries, mergedGeom, width, height, depth } = await initializeModel({ scene });
...
async function initializeModel({ scene }) {
    ...
    model.scene.traverse((child) => {
        if (child.isMesh) {
            ...
            // Aligne le bas du modèle 3D sur le bas du corps physique (-center.x/z pour l'axe vertical)
...
function createInstance({ scene, instances, colliderData: { vertices, indices }, groups }) {
    ...
    // Collider ConvexHull pour reproduire le contour exact de Boopy (permet de rouler / basculer)
