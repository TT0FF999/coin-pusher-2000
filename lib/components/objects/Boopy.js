...
// Extraction dynamique de tous les maillages du modèle GLB
async function initializeModel({ scene }) {
    const model = await scene.loadModel(MODEL_PATH);
    const materials = [];
    const geometries = [];

    // Parcourt tous les éléments 3D du fichier boopy.glb
    ...
    // Calcul automatique des dimensions exactes à partir du modèle 3D
    const merged = scene.mergeGeometries(geometries);
